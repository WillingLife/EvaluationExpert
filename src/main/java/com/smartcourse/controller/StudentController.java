...
public class StudentController {
    ...
    public Result<AssignmentScoreUploadVO> submit(@RequestPart("meta") StudentSubmitMetaDTO studentSubmitMetaDTO,
    ...
    public Result<DifyGradeAssignmentVO> feedback(@RequestParam("student_id") Long studentId,
                           @RequestParam("assignment_id") Long assignmentId) {
        log.info("学生查看作业评语 studentId:{} assignmentId:{}", studentId, assignmentId);
        DifyGradeAssignmentVO feedback = assignmentService.getFeedback(studentId, assignmentId);
    ...
    public Result<List<AssignmentListItemVO>> listAssignments(StudentAssignmentListDTO dto) {
        log.info("学生查询作业列表：{}", dto);
        List<AssignmentListItemVO> items = assignmentService.listStudentAssignments(dto);
        return Result.success(items);
    }


    @GetMapping("/assignment")
    public Result<AssignmentVO> getAssignment(@RequestParam("assignment_id") Long assignmentId) {
